...
export default function Blog() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: darkMode ? 'linear-gradient(135deg, #0f0f23 0%, #1a1a3e 50%, #0d1b2a 100%)' : '#f8f9fa', color: darkMode ? '#fff' : '#1a1a3e', fontFamily: 'system-ui, sans-serif', padding: '2rem' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto', paddingTop: '2rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          <div>
            ...
            <p style={{ color: darkMode ? '#9ca3af' : '#6b7280' }}>{allPosts.length} articles on AI</p>
          </div>
        ...
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
          <input type="text" placeholder="Search..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ flex: '1 1 200px', padding: '0.75rem', borderRadius: '8px', border: '1px solid #374151', background: darkMode ? 'rgba(255,255,255,0.05)' : '#fff', color: darkMode ? '#fff' : '#1a1a3e' }} />
          <select value={category} onChange={(e) => setCategory(e.target.value)} style={{ padding: '0.75rem', borderRadius: '8px', border: '1px solid #374151', background: darkMode ? 'rgba(255,255,255,0.05)' : '#fff', color: darkMode ? '#fff' : '#1a1a3e' }}>
        ...
        <div style={{ display: 'grid', gap: '1rem' }}>
          {filteredPosts.map(post => (
            <a key={post.slug} href={`/blog/${post.slug}`} style={{ display: 'block', background: darkMode ? 'rgba(255,255,255,0.03)' : '#fff', padding: '1.25rem', borderRadius: '12px', border: darkMode ? '1px solid rgba(255,255,255,0.05)' : '1px solid #e5e7eb', textDecoration: 'none', color: 'inherit' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <span style={{ fontSize: '0.75rem', color: darkMode ? '#6b7280' : '#9ca3af' }}>{post.date}</span>
              </div>
            </a>
          ))}
        </div>
